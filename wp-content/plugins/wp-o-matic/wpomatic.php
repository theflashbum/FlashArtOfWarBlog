...
class WPOMatic {               
  ...
  function processItem(&$campaign, &$feed, &$item)
  {
    ...
    // Create post
    $postid = $this->insertPost($wpdb->escape($item->get_title()), $content, $date, $categories, $campaign->posttype, $campaign->authorid, $campaign->allowpings, $campaign->comment_status, $meta);
...
function myTruncate($string, $limit, $break=".", $pad="...")
{
  ...
  {  
    ...
    $replace = array(
      ...
      $campaign->id,
      $campaign->slug,
	  $this->myTruncate(strip_tags($content), 500)
    );
