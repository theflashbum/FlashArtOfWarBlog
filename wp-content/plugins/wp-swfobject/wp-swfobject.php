...
$wpswf_version	= "2.2";										// version of plugin 
...
function wpswfObject($match) {
	...
	if ($swf_tube["host"] == "www.youtube.com") {
		...
		if ($wpswf_config['swf_showinfo'] == "false") {
			$swf_path.= "&showinfo=0";
		}
	}
	...
	if ($swf_file != "vxhtml") {
		...
		if ($tmpalign != "") {
			$writeswf = "<div style=\"text-align:center; width:".$swf_width."px; height:".$swf_height."px; line-height:".$swf_height."px; ".$tmpalign.";\">".$writeswf."</div>\n";
		}
	}
	// Write code for SWF
	switch ($swf_file) {
		case "iphone":
			...
			if ($swf_tubekey != "") {
				$writeswf.= "<div id=\"swf".$wpswf_random.$wpswf_number."\"><a href=\"http://www.youtube.com/watch?v=".$swf_tubekey."\"><img src=\"http://img.youtube.com/vi/".$swf_tubekey."/default.jpg\" width=\"".$swf_width."\" height=\"".$swf_height."\" border=\"0\"></a></div>";
			} else {
		...
		case "vxhtml":
			...
			$writeswf.= "<param name=\"movie\" value=\"".$swf_path."\" />\n";
			$writeswf.= "<param name=\"quality\" value=\"".$wpswf_config['swf_quality']."\" />\n";
			$writeswf.= "<param name=\"wmode\" value=\"".$wpswf_config['swf_wmode']."\" />\n";
			$writeswf.= "<param name=\"menu\" value=\"".$wpswf_config['swf_menu']."\" />\n";
			$writeswf.= "<param name=\"bgcolor\" value=\"".$wpswf_config['swf_bgcolor']."\" />\n";
			if ($wpswf_config['swf_fullscreen'] == "true") {
